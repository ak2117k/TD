...
export interface AdaptiveStopOpenTradeInput {
  instrumentId: string;
  side: 'BUY' | 'SELL';
  quantity: number;
  entryPrice: number;
...
export interface AdaptiveStopCloseTradeOpts {
  reason: string;
  exitPrice: number;   // REQUIRED on adaptive-stop track
 ...
 */
@Injectable()
export class AdaptiveStopTradeExecutionService {
  private readonly logger = new Logger(AdaptiveStopTradeExecutionService.name);
  ...
  async openTrade(input: AdaptiveStopOpenTradeInput) {
  ...
  async closeTrade(tradeId: string, opts: AdaptiveStopCloseTradeOpts) {
    const trade = await this.trades.getTradeById(tradeId);
    if (!trade) throw new Error(`AdaptiveStopTrade ${tradeId} not found`);
