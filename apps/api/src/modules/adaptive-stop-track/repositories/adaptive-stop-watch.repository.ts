...
import { Prisma, AdaptiveStopWatchEntry, WatchEventType, WatchStatus } from '@prisma/client';
import { PrismaService } from '../../../common/prisma/prisma.service';

export interface AdaptiveStopCreateEntryInput {
  alertId: string | null;
  setupId: string | null;
  symbol: string;
  token: string;
 ...
 * Keep correctness changes in sync. See specs/2026-06-09-adaptive-stop-track-design.md.
 *
 ...
 *    adaptive-stop-trade.repository.ts + the controller does the join.
 *  - createEntry accepts optional ATR-based stop sizing fields.
 */
@Injectable()
export class AdaptiveStopWatchRepository {
  constructor(private readonly prisma: PrismaService) {}

  async createEntry(input: AdaptiveStopCreateEntryInput): Promise<AdaptiveStopWatchEntry> {
    return this.prisma.adaptiveStopWatchEntry.create({
      data: {
  ...
  async createEvent(input: AdaptiveStopCreateEventInput) {
    return this.prisma.adaptiveStopWatchEvent.create({
      data: {
  ...
  async findById(id: string): Promise<AdaptiveStopWatchEntry | null> {
    return this.prisma.adaptiveStopWatchEntry.findUnique({ where: { id } });
  }

  async findByIdWithEvents(id: string) {
    return this.prisma.adaptiveStopWatchEntry.findUnique({
      where: { id },
      include: { events: { orderBy: { createdAt: 'desc' }, take: 100 } },
    });
  }

  async findActiveByToken(token: string): Promise<AdaptiveStopWatchEntry[]> {
    return this.prisma.adaptiveStopWatchEntry.findMany({
      where: { token, status: { notIn: CLOSED_STATES } },
    });
  }

  async findAllActive(): Promise<AdaptiveStopWatchEntry[]> {
    return this.prisma.adaptiveStopWatchEntry.findMany({
  ...
  async countActive(): Promise<number> {
    return this.prisma.adaptiveStopWatchEntry.count({
      where: { status: { notIn: CLOSED_STATES } },
    });
  }
  ...
  async wasTokenExecutedSince(token: string, since: Date): Promise<boolean> {
    const n = await this.prisma.adaptiveStopWatchEntry.count({
  ...
  async list(opts: { status?: WatchStatus; date?: string }) {
    const where: Prisma.AdaptiveStopWatchEntryWhereInput = {};
  ...
  async getLastClosedPnlForToken(token: string, since?: Date): Promise<number | null> {
    const entry = await this.prisma.adaptiveStopWatchEntry.findFirst({
      where: {
        token,
    ...
    const trade = await this.prisma.adaptiveStopTrade.findUnique({
  ...
  async update(id: string, data: Prisma.AdaptiveStopWatchEntryUpdateInput) {
    return this.prisma.adaptiveStopWatchEntry.update({ where: { id }, data });
  }

  /**
   * Resolve alertId -> Chartink scanner name. Batched; deduped; null-safe.
   * Mirrors WatchRepository.findScannerNames — the adaptive-stop controller does the
